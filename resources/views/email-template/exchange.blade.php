...
<body>
    ...
    <p>We have received the 
        @if ($invoice->currency == "Bitcoin")
            {{number_format(($invoice->order_amount/getCurrentBtcDollar()), 6)}} BTC 
        @else
            ${{number_format(($invoice->order_amount), 2)}}
        @endif
    exchange request placed on abijanexchange.com.</p>
    ...
    <p>You are Selling ${{number_format($invoice->order_amount, 2)}} worth of Bitcoin Cash for {{number_format(($invoice->order_amount*rates()[4]['sell_rate']), 2)}} Naira</p>
    ...
    <p>You are Selling ${{number_format($invoice->order_amount, 2)}} worth of Ethereum for {{number_format(($invoice->order_amount*rates()[2]['sell_rate']), 2)}} Naira</p>
    ...
    @endif
    @if ($invoice->currency == "Bitcoin")
    <p>You are Selling ${{number_format($invoice->order_amount, 2)}} worth of Bitcoin for {{number_format(($invoice->order_amount*rates()[0]['sell_rate']), 2)}} Naira</p>
    ...
    <p>You are Selling ${{number_format($invoice->order_amount, 2)}} worth of USDT TRC20 for {{number_format(($invoice->order_amount*rates()[3]['sell_rate']), 2)}} Naira</p>
    ...
    <p>=============================================================</p>
    <p>
        USD AMT: {{number_format($invoice->order_amount, 2)}} <br>
        @if ($invoice->currency == "Bitcoin")
             BTC VALUE: {{number_format(($invoice->order_amount/getCurrentBtcDollar()), 6)}} <br>
        @endif
